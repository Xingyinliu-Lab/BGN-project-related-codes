...
data<- read.table("beta_group.csv", header = TRUE, sep = "," )
data$group<-data$subject
data$sample<-data$subid
...
groups<-data.frame(data$subid,data$group)
...
otu.adonis2=adonis2(dis~V2,data = groups,distance = "bray")
...
p<-ggplot(plotdata, aes(PC1, PC2)) +
  ...
  stat_ellipse(aes(x = PC1, y = PC2, fill = group), geom = "polygon", alpha = 1/2, level = 0.90)+ 
  scale_fill_nejm()+
  scale_color_nejm()+
